...
RUN curl -L http://nginx.org/download/nginx-1.19.8.tar.gz -o /tmp/nginx-1.19.8.tar.gz \
    && echo "308919b1a1359315a8066578472f998f14cb32af8de605a3743acca834348b05 /tmp/nginx-1.19.8.tar.gz" | sha256sum -c - \
    && tar xvfz /tmp/nginx-1.19.8.tar.gz -C /tmp
...
RUN cd /tmp/nginx-1.19.8 \
    && /tmp/nginx-1.19.8/configure \
        --prefix=/opt/nginx \
